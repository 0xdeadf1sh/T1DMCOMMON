...
3. `../<PROJECT>/CLAUDE.md` — its local rules.
4. `../<PROJECT>/.claude/skills/` — its skills. Several are **mandatory gates**;
   read their descriptions before you start.
...
Two cautions on the layout. `T1DMDROID`'s skills are mandatory gates in their own
right, and its build and branch discipline live in its `CLAUDE.md` rather than
here. And `../T1DMDROID-vk-build` is a build variant, not the project — do not
work in it.
...
Every sister project's memory is invisible until you read it, and some of it is
load-bearing — that the model consumes carbohydrate as an appearance curve rather
than a delivery schedule lives in a memory, not in any source file.

**On demand, not in bulk.** Each directory has a `MEMORY.md` index, one line per
memory. Read the index and open only the entries your task touches. There are
more than twenty for `T1DMDROID` alone.

Three cautions:

- **A memory records what was true when it was written.** It is not verified
  against the code. If one names a file, a constant, a device, or a decision,
  confirm it still holds. Where a memory and this specification disagree, this
  specification wins; where a memory and the code disagree, the code wins and the
  memory is a defect.
- **Some projects have memories under an older path encoding**, left behind when
  ...
  under `-home-omar-<PROJECT>/`, largely superseded and in places contradicting
  the current set. Prefer the `-home-omar-Desktop-` directory.
...
Every divergence this suite has suffered began as a local copy: a constant
inlined "just here", a contract document duplicated into a second repository, a
numeric routine ported rather than shared. Both copies were correct on the day
they were written and stopped agreeing later, silently, because nothing was
watching.
...
A stale specification is worse than none: it is trusted, read first, and sends
...
claim. An unreported staleness is indistinguishable from a fact.

**State what is true now.** Every file here describes the suite as it stands, in
the present tense. When an implementation catches up with the specification,
**delete** the entry: do not annotate it "Resolved", do not leave the old
behaviour beside the new one, and do not narrate what changed. The same holds for
prose: describe the mechanism in place, not the one it replaced — unless the
...
Where a calculation could mislead if wrong, prefer failing closed — withholding a
number — over showing a value you cannot justify.
